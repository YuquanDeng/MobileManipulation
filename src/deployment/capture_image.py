...
"""
This is the new version of capture_image.py python script aim for logging short goal images
more precisely. The pipeline records the images in x Hz defined in the spot_config.yaml.
For previous capture_image.py, please check the previous commit. The previous image data
acquisition pipeline captures the images manually. 
"""
...
class WasdInterface(object):
    ...
    def __init__(self, robot, params):
        ...
        self._task_list = [self._robot_state_task, self._image_task]
        ...
        self._command_dictionary = {
            ...
            ord('I'): self._start_capture_images,
            ord('O'): self._stop_capture_images
        }
    ...
    def start(self):
        ...
        update_thread = Thread(target=self._update_thread, args=[self._async_tasks])
    ...
    def _start_capture_images(self):
        ...
        LOGGER.info("Start image capturing...")
    
    def _stop_capture_images(self):
        """Stop image capture process."""
        self._stop_capture = True
        LOGGER.info("Stop image capturing...")
            ...
            for im_resp in get_im_resp:
                ...
                if im_resp.source.name == 'hand_color_image':
                    try:
                        ...
                        img_name = f"{self._img_index}.jpg"
                        obs_img.save(os.path.join(self._dir, img_name))
                        self._img_index += 1
    ...
    def _drive_draw(self, stdscr, lease_keep_alive):
        ...
        stdscr.addstr(12, 0, "          [I]: Start image capture, [O]: Stop image capture")
    ...
    def _move_forward(self):
        self._velocity_cmd_helper('move_forward', v_x=self._params["VELOCITY_BASE_SPEED"])

    def _move_backward(self):
        self._velocity_cmd_helper('move_backward', v_x=-self._params["VELOCITY_BASE_SPEED"])

    def _strafe_left(self):
        self._velocity_cmd_helper('strafe_left', v_y=self._params["VELOCITY_BASE_SPEED"])

    def _strafe_right(self):
        self._velocity_cmd_helper('strafe_right', v_y=-self._params["VELOCITY_BASE_SPEED"])

    def _turn_left(self):
        self._velocity_cmd_helper('turn_left', v_rot=self._params["VELOCITY_BASE_ANGULAR"])

    def _turn_right(self):
        self._velocity_cmd_helper('turn_right', v_rot=-self._params["VELOCITY_BASE_ANGULAR"])
    ...
    def _velocity_cmd_helper(self, desc='', v_x=0.0, v_y=0.0, v_rot=0.0):
        self._start_robot_command(
            ...
            end_time_secs=time.time() + self._params["VELOCITY_CMD_DURATION"])
...
def _setup_logging(verbose):
    ...
    # TODO: comment this out for saving log messages to file wasd.log for later debugging.
...
def main(cfg):
    ...
    stream_handler = _setup_logging(cfg["verbose"])

    # Create robot object.
    sdk = create_standard_sdk('WASDClient')
    robot = sdk.create_robot(cfg["hostname"])
    try:
        robot.authenticate(username=cfg["username"], password=cfg["password"])
        robot.start_time_sync(cfg["time_sync_interval_sec"])
    except RpcError as err:
        LOGGER.error("Failed to communicate with robot: %s" % err)
        return False

    wasd_interface = WasdInterface(robot, cfg)
    try:
